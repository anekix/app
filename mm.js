...
function make_year_selector(){
	...
	var year_selectList = document.createElement("select");
	year_selectList.setAttribute("id", "year_selectList");
	year_selectList.addEventListener("change",create_canvas, false);

	for (var j = 0; j < year.length; j++) {
		var option = document.createElement("option");
		option.setAttribute("value", year[j]);
		option.text = year[j];
		year_selectList.appendChild(option);
	}

	var foo = document.getElementById("picker");
	foo.appendChild(year_selectList)
...
function create_canvas() {
	...
	year_selected = parseInt(this.value);
